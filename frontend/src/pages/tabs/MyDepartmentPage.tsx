...
import { DepartmentIcon, PlusIcon, SearchIcon, UsersIcon, TrashIcon, CheckIcon, UserIcon, CalendarIcon, ChevronRightIcon, BuildingIcon, DoorIcon, CheckCircleIcon, ClockIcon } from '../../components/Icons'
        ...
        />
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 mb-6">
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3.5 sm:gap-4 transition-all duration-300">
                    
                    {/* 1. Identity & Team Distribution Hero Card (Spans 4 cols on lg) */}
                    <div className="col-span-1 md:col-span-2 lg:col-span-4 bg-gradient-to-br from-[var(--brand-color)] via-[#526f34] to-[#29361c] rounded-3xl p-5 sm:p-6 min-h-[10.5rem] flex flex-col justify-between relative overflow-hidden group/hero shadow-md border border-white/10 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                      {/* Dotted Pattern Overlay */}
                      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:16px_16px] opacity-[0.15] group-hover/hero:opacity-[0.25] transition-opacity duration-500" />
                      {/* Ambient Blur Ellipse */}
                      <div className="absolute -top-32 -right-10 w-96 h-64 bg-emerald-400/40 blur-[64px] rounded-[100%] -rotate-12 group-hover/hero:bg-emerald-400/60 group-hover/hero:scale-110 transition-all duration-700 pointer-events-none" />
                      
                      {/* Top Bar: Dept Code & Member Avatar Stack */}
                      <div className="flex items-center justify-between relative z-10 mb-2 gap-2">
                        {departmentInfo?.code ? (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/30 backdrop-blur-md border border-white/20 text-white text-[0.6875rem] font-black tracking-wider uppercase shadow-xs">
                          ...
                          <span className="text-[0.65rem] font-bold text-white/70 uppercase tracking-wider bg-black/20 px-2.5 py-1 rounded-full border border-white/10">
                            Department
                          </span>
                        )}
                        ...
                        <div className="flex items-center gap-1">
                          <div className="flex -space-x-2 overflow-hidden">
                            {members.slice(0, 4).map((m) => (
                              <div key={m.id} className="h-7 w-7 sm:h-8 sm:w-8 rounded-full border-2 border-white/80 overflow-hidden bg-white/20 shadow-xs shrink-0" title={m.name || m.email}>
                                {m.avatar && !avatarErrors[m.avatar] ? (
                                  <img 
                                    src={m.avatar} 
                                    alt={m.name} 
                                    className="h-full w-full object-cover" 
                                    onError={() => setAvatarErrors(prev => ({...prev, [m.avatar]: true}))} 
                                  />
                                ) : (
                                  <div className="h-full w-full flex items-center justify-center text-white/90 bg-white/20">
                            ...
                            {members.length > 4 && (
                              <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full border-2 border-white/80 bg-black/40 backdrop-blur-md flex items-center justify-center shadow-xs shrink-0">
                                <span className="text-[0.625rem] font-black text-white">+{members.length - 4}</span>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {/* Main Info: Logo + Department Name */}
                      <div className="flex items-center gap-3 relative z-10 my-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <h2 className="text-base sm:text-lg font-black text-white leading-tight truncate drop-shadow-xs">
                            {loading ? 'Loading...' : (departmentInfo?.name || 'Unassigned Department')}
                          </h2>
                          <p className="text-[0.6875rem] font-semibold text-emerald-300/90 flex items-center gap-1.5 mt-0.5">
                            <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                            <span>{activeCount} Active Member{activeCount === 1 ? '' : 's'}</span>
                            <span className="text-white/40">•</span>
                            <span className="text-white/80">{totalMembers} Total</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="relative z-10 space-y-2 my-1">
                        {(() => {
                          ...
                          const getPhaseBadge = (phase: string) => {
                            switch(phase) {
                              case 'Drafting': return 'text-amber-700 bg-amber-50 border-amber-200';
                              case 'Plotting': return 'text-blue-700 bg-blue-50 border-blue-200';
                              case 'Revision': return 'text-purple-700 bg-purple-50 border-purple-200';
                              case 'Final': return 'text-emerald-700 bg-emerald-50 border-emerald-200';
                        ...
                        {topScheduleDays.slice(0, 3).map((item, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex flex-col items-center justify-center font-black text-xs sm:text-sm text-emerald-950 uppercase tracking-tighter leading-none py-1 my-auto">
                                {dayChars.map((char, cIdx) => (
                                  <span key={cIdx} className="block group-hover/pillar:scale-110 transition-transform my-0.5">{char}</span>
                                ))}
                              </div>
                            </div>
                          );
                        ...
                        {displayBuildings.slice(0, 2).map((b, idx) => {
                          ...
                          return (
                            <div key={b.id || idx} className="flex-1 flex items-center justify-between p-2 px-2.5 rounded-xl bg-blue-50/80 border border-blue-200 shadow-2xs hover:border-blue-300 hover:shadow-xs transition-all duration-300">
                              ...
                              <span className="text-[0.625rem] font-black text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-md shrink-0 shadow-2xs">
                                {roomCount} {roomCount === 1 ? 'Room' : 'Rooms'}
                              </span>
                            </div>
                          );
                        })}
                        {displayBuildings.length > 2 && (
                          <p className="text-[0.6rem] font-bold text-blue-600 text-right pr-1">
                            +{displayBuildings.length - 2} more building{displayBuildings.length - 2 === 1 ? '' : 's'}
                          </p>
                        )}
                      </div>
